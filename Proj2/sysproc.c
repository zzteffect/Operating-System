...
//set the flag ; add knel code in proc.c
  return 0;
}

/*
  arg = 0 -> default policy is adopted
  arg = 1 -> stride scheduling policy is used
*/

extern int policy;//policy = 0 -> default policy is adopted; policy = 1 -> stride scheduling policy is used
...
/*
  return the # of tickets owned by the process
  find the process by using its pid
*/

extern int getTickets(int);
...
extern int transferTickets(int, int);
int sys_transfer_tickets(void){
  int p; //recipient's process pid
  argint(0, &p);
  int numTickets; //# of tickets that want to transfer
  argint(1, &numTickets);
  return transferTickets(p, numTickets);
}
