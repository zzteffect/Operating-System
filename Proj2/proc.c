...
rebalance(void){
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state == RUNNABLE || p->state == RUNNING ){
        p->ticketsNum = (100)/activeProcess;
        p->pass = 0;
        p->stride = (100*10)/p->ticketsNum;
      }
  }
}
...
int 
transferTickets(int pid, int transferT){
  struct proc *p;
  struct proc *curP = myproc();

  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){  
    if(pidExist(pid) == 0){//pid does not exist
        return -3;
      }
    ...
    if(transferT > (p->ticketsNum -1)){
      return -2;
    }
    
    if(pid == p->pid){
      p->ticketsNum += transferT;
      p->stride = (100*10)/p->ticketsNum;
      curP->ticketsNum -= transferT;
      curP->stride = (100*10)/curP->ticketsNum;
      return curP->ticketsNum;
    }
  }
